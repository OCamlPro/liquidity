...
module Compile = LiquidOCamlCompile
  ...
  let open Ast_helper in
  {
    ...
    expr = (fun mapper expr ->
      ...
      | Pexp_constant (
         ...
         ->
           Exp.apply ~loc (Exp.ident ~loc
                        { txt = Ldot(Lident "Int",
                                     "of_string"); loc })
                     [Nolabel, Exp.constant ~loc (Pconst_string (s,None))]
      | Pexp_constant (
                  Pconst_integer (s, Some '\231')
              )
        ->
         Exp.apply ~loc (Exp.ident ~loc
                      { txt = Ldot(Lident "Tez",
      ...
      | Pexp_construct (
        ...
        ->
         Exp.apply ~loc (Exp.ident ~loc
                      { txt = Ldot(Lident "Map",
                            "empty"); loc })
                   [Nolabel, Exp.constant ~loc (Pconst_integer ("0",None))]
