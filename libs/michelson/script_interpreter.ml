      ...
      fun origination qta ctxt ({ instr ; loc } as descr) stack ->
        if Compare.Int.(qta <= 0) then
          ...
          | Add_seconds_to_timestamp, Item (n, Item (t, rest)) ->
             begin
             match Script_int.to_int64 n with
          ...
          | Ediv_tez, Item (x, Item (y, rest)) ->
             let x = Script_int.of_int64 (Tez.to_cents x) in
             let y = Script_int.of_int64 (Tez.to_cents y) in
             let result = Script_int.ediv x y in
             logged_return (Item (result, rest), qta -1, ctxt)
          ...
          | Ediv_natnat, Item (x, Item (y, rest)) ->
               logged_return (Item (Script_int.ediv x y, rest), qta -1, ctxt)
          | Lsl_int, Item (x, Item (y, rest)) ->
               if Compare.Int.(Script_int.compare y (Script_int.of_int64 256L) >= 0 ) then
                 fail (Overflow loc)
               else
                 logged_return (Item (Script_int.shift_left x y, rest), qta - 1, ctxt)
          | Lsr_int, Item (x, Item (y, rest)) ->
              logged_return (Item (Script_int.shift_right x y, rest), qta - 1, ctxt)
          | Or_int , Item (x, Item (y, rest)) ->
              logged_return (Item (Script_int.logor x y, rest), qta - 1, ctxt)
          | And_int, Item (x, Item (y, rest)) ->
              logged_return (Item (Script_int.logand x y, rest), qta - 1, ctxt)
          | Xor_int, Item (x, Item (y, rest)) ->
          ...
          | Compare Bool_key, Item (a, Item (b, rest)) ->
              ...
              let cmpres = Script_int.of_int64 (Int64.of_int cmpres) in
          ...
          | Steps_to_quota, rest ->
              let steps = Script_int.of_int64 (Int64.of_int qta) in
