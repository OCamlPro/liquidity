...
type 'ty comparable_ty =
  | Int_key : (z int_val) comparable_ty
  | Nat_key : (nat int_val) comparable_ty
...
and ('bef, 'aft) instr =
  ...
  | Add_seconds_to_timestamp :
    (nat int_val * (Timestamp.t * 'rest), Timestamp.t * 'rest) instr
  | Add_timestamp_to_seconds :
    (Timestamp.t * (nat int_val * 'rest), Timestamp.t * 'rest) instr
    (* currency operations *)
  ...
  | Sub_tez :
      (Tez.t * (Tez.t * 'rest), Tez.t * 'rest) instr
  | Mul_teznat :
      (Tez.t * (nat int_val * 'rest), Tez.t * 'rest) instr
  | Mul_nattez :
      (nat int_val * (Tez.t * 'rest), Tez.t * 'rest) instr
  | Ediv_teznat :
      (Tez.t * (nat int_val * 'rest),
             ((Tez.t, Tez.t) pair) option * 'rest) instr
  | Ediv_tez :
      (Tez.t * (Tez.t * 'rest),
             ((nat int_val, nat int_val) pair) option * 'rest) instr
  ...
  | Not :
   ...
   (* integer operations *)
  | Neg_nat : (nat int_val * 'rest, z int_val * 'rest) instr
  | Neg_int : (z int_val * 'rest, z int_val * 'rest) instr

  | Abs_int : (z int_val * 'rest, nat int_val * 'rest) instr
  | Int_nat : (nat int_val * 'rest, z int_val * 'rest) instr

  | Add_intint : (z int_val * (z int_val * 'rest), z int_val * 'rest) instr
  | Add_intnat : (z int_val * (nat int_val * 'rest), z int_val * 'rest) instr
  | Add_natint : (nat int_val * (z int_val * 'rest), z int_val * 'rest) instr
  | Add_natnat : (nat int_val * (nat int_val * 'rest), nat int_val * 'rest) instr

  | Sub_int : ('s int_val * ('t int_val * 'rest), z int_val * 'rest) instr


  | Mul_intint : (z int_val * (z int_val * 'rest), z int_val * 'rest) instr
  | Mul_intnat : (z int_val * (nat int_val * 'rest), z int_val * 'rest) instr
  | Mul_natint : (nat int_val * (z int_val * 'rest), z int_val * 'rest) instr
  | Mul_natnat : (nat int_val * (nat int_val * 'rest), nat int_val * 'rest) instr

  | Ediv_intint : (z int_val * (z int_val * 'rest),
             ((z int_val, nat int_val) pair) option * 'rest) instr
  | Ediv_intnat : (z int_val * (nat int_val * 'rest),
             ((z int_val, nat int_val) pair) option * 'rest) instr
  | Ediv_natint : (nat int_val * (z int_val * 'rest),
             ((z int_val, nat int_val) pair) option * 'rest) instr
  | Ediv_natnat : (nat int_val * (nat int_val * 'rest),
             ((nat int_val, nat int_val) pair) option * 'rest) instr

  | Lsl_int : (nat int_val * (nat int_val * 'rest), nat int_val * 'rest) instr
  | Lsr_int : (nat int_val * (nat int_val * 'rest), nat int_val * 'rest) instr
  | Or_int : (nat int_val * (nat int_val * 'rest), nat int_val * 'rest) instr
  | And_int : (nat int_val * (nat int_val * 'rest), nat int_val * 'rest) instr
  | Xor_int : (nat int_val * (nat int_val * 'rest), nat int_val * 'rest) instr
  | Not_nat : (nat int_val * 'rest, z int_val * 'rest) instr
  | Not_int : (z int_val * 'rest, z int_val * 'rest) instr
  (* control *)
  ...
  | Compare : 'a comparable_ty ->
    ('a * ('a * 'rest), z int_val * 'rest) instr
  (* comparators *)
  | Eq :
      (z int_val * 'rest, bool * 'rest) instr
  | Neq :
      (z int_val * 'rest, bool * 'rest) instr
  | Lt :
      (z int_val * 'rest, bool * 'rest) instr
