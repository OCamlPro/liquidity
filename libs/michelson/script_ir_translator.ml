...
let rec parse_data
  ...
  = fun ctxt ty script_data ->
    ...
    (* Integers *)
    | Int_t, Int (_, v) -> begin try
          return (Script_int.of_string v)
        with _ -> fail (error ())
      end
    | Nat_t, Int (_, v) -> begin try
          let v = Script_int.of_string v in
...
and parse_instr
  ...
  fun ctxt ?storage_type script_instr stack_ty ->
    ...
    | Prim (loc, "LSL", []),
      Item_t (Nat_t, Item_t (Nat_t, rest)) ->
        return (typed loc (Lsl_int, Item_t (Nat_t, rest)))
    | Prim (loc, "LSR", []),
      Item_t (Nat_t, Item_t (Nat_t, rest)) ->
        return (typed loc (Lsr_int, Item_t (Nat_t, rest)))
    | Prim (loc, "OR", []),
      Item_t (Nat_t, Item_t (Nat_t, rest)) ->
        return (typed loc (Or_int, Item_t (Nat_t, rest)))
    | Prim (loc, "AND", []),
      Item_t (Nat_t, Item_t (Nat_t, rest)) ->
        return (typed loc (And_int, Item_t (Nat_t, rest)))
    | Prim (loc, "XOR", []),
      Item_t (Nat_t, Item_t (Nat_t, rest)) ->
        return (typed loc (Xor_int, Item_t (Nat_t, rest)))
    | Prim (loc, "NOT", []),
      Item_t (Int_t, rest) ->
