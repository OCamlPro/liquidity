...
let ed25519_public_key_hash_tezos = "\006\161\159" (* tz1(36) *)
let ed25519_public_key_hash_dune  = "\004\177\001" (* dn1(36) *)

module Public_key_hash_tezos = struct
  include Blake2B.Make(Base58)(struct
    let name = "Ed25519.Public_key_hash"
    let title = "An Ed25519 public key ID"
    let b58check_prefix = ed25519_public_key_hash_tezos
    let size = Some 20
  end)
  let of_public_key v =
    hash_bytes
...
module Public_key_hash_dune = struct
  include Blake2B.Make(Base58)(struct
    let name = "Ed25519.Public_key_hash"
    let title = "An Ed25519 public key ID"
    let b58check_prefix = ed25519_public_key_hash_dune
    let size = Some 20
  end)
  let of_public_key v =
    hash_bytes
